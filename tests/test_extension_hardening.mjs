...
// --- popup guard (hijack-click popup/popunder ad tabs) ---
{
  ...
  ok("content arms the guard on pointerdown AND click (mousedown popunders)", /addEventListener\("pointerdown", reportGesture, true\)/.test(content) && /addEventListener\("click", reportGesture, true\)/.test(content));
  ok("gesture report carries the anchor's declared destination", /anchorHost/.test(content) && /minus:nonlink-click/.test(content));
